...
export default function ResaleHubCancellationsScreen() {
  ...
  const load = useCallback(() => {
    apiGet<StoreOrder[] | { error: string }>("/api/store-orders?mine=1&canceled=1")
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable
            ...
            onPress={() =>
              router.push(
                `/web?url=${encodeURIComponent(`${siteBase}/resale-hub/cancellations`)}&title=${encodeURIComponent("Cancellations")}` as never
              )
            }
          >
