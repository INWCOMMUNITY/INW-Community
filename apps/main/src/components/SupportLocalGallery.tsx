...
export function SupportLocalGallery() {
  ...
  return (
    <>
      {/* Phone / narrow tablet: app-style directory */}
      <SupportLocalGalleryMobile
      ...
      {/* Desktop (1024px+): original directory — hero/tan live on page.tsx */}
      <SupportLocalGalleryDesktop
        businesses={businesses}
        search={search}
        setSearch={setSearch}
        category={category}
...
function SupportLocalGalleryMobile({
  businesses,
  search,
  setSearch,
  category,
  ...
  const grid = loading ? (
    ...
    <div className="grid grid-cols-2 gap-3">
      {businesses.map((b) => (
        <BusinessCardMobile key={b.id} business={b} saved={savedIds.has(b.id)} />
      ))}
    </div>
  );
...
/** Pre–mobile-web directory UI (dropdown filters + coupon-style cards). */
function SupportLocalGalleryDesktop({
  businesses,
  search,
  setSearch,
  category,
                    ...
                    <div className="flex-1 min-w-0 relative">
                      <div className="flex items-start gap-1">
                        ...
                        <button
                          ...
                          aria-label={
                            expandedId === b.id ? "Collapse description" : "Show full description"
                          }
                        >
                          <svg
