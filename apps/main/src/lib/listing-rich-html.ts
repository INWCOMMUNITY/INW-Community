 ...
 * so storefront HTML can display without inheriting remote typography.
 *
 ...
 * Handles listing descriptions that encode apostrophes, quotes, dashes, etc.
 */
function decodeHtmlEntities(html: string): string {
  ...
  // Decode common named entities used in listing descriptions
...
function sanitizeHtml(html: string): string {
  ...
  result = result.replace(/<\/?([a-z][a-z0-9]*)\b[^>]*\/?>/gi, (match, tagName) => {
    ...
    // Remove disallowed tags (like span, font, a) but keep the text between them
    return "";
  });
...
function descriptionLooksLikeHtml(value: string | null | undefined): boolean {
...
/** Sanitize listing HTML for storage and storefront display. */
