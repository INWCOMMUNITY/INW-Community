...
import type { ChannelConnectionContext, ChannelProvider, SyncStoreItem } from "./types";
...
export async function deleteStoreItemFromChannels(storeItemId: string): Promise<void> {
  const links = await prisma.channelListingLink.findMany({
    where: { storeItemId },
    include: { connection: true },
  });
