...
export async function refreshEbayListingByItemId(
  ...
  if (preserveInwContent && opts?.skipContent !== true) {
    console.log("[ebay] refreshEbayListingByItemId: keep INW title; still apply description diffs", {
      storeItemId: storeItem.id,
      legacyItemId,
      source: opts?.source ?? "cron",
      inwTitle: storeItem.title,
  ...
  if (Object.keys(updateData).length > 0) {
    ...
    // eBay variation qty propagation can take 10-20+ minutes, so use 15 min delay.
    if (changes.some((c) => c.includes("variants (qty)")) && link.id) {
      const delayMs = 15 * 60_000; // 15 minutes - eBay variation qty lag is significant
...
/**
 * Apply title only from a verified notification snapshot when GetItem failed.
 * Never writes qty, price, photos, description, aspects, variations, or Ended.
 */
...
/**
 * Save-time INW→eBay is the primary push. Cron only retries failed writes.
 * Do not treat storeItem.updatedAt as an outbound save — shipping attach and
 * other GetItem side effects bump it and would snap INW qty over Seller Hub.
 */
...
export async function pushFailedEbayOutboundForConnection(
  ...
  const links = errored.filter((link) => {
