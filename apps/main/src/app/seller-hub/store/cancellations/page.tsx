...
export default function CancellationsPage() {
  ...
  return (
    <div>
      ...
      {fetchError && (
        <div className="border rounded-lg p-6 bg-red-50 mb-8">
          ...
          {fetchError.includes("sign in") && (
            <Link href="/login?callbackUrl=/seller-hub/store/cancellations" className="btn mt-4 inline-block">
              Sign in
            </Link>
          )}
        </div>
      ...
      <Link href="/seller-hub/store/items" className="inline-block mt-6 hover:underline" style={{ color: "var(--color-link)" }}>
        View My Items →
      </Link>
    </div>
  );
